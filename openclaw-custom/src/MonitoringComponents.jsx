...
export const AgentHealthMonitor = ({ agents, data, isExpanded, onToggle }) => {
  const normalizedHealthData = useMemo(() => {
    ...
    const derivedAgents = fallbackAgents.map((agent) => ({
      name: agent.name,
      health: agent.status === 'awake' || agent.status === 'working' ? 'healthy' : 'idle',
      lastHeartbeat: agent.status === 'awake' || agent.status === 'working' ? 'just now' : '1m ago',
      responseTime: agent.status === 'awake' || agent.status === 'working' ? '180ms' : '1.1s',
      successRate: agent.status === 'awake' || agent.status === 'working' ? 98 : 88,
      cpuUsage: agent.status === 'awake' || agent.status === 'working' ? 18 : 42,
      tasks: agent.status === 'awake' || agent.status === 'working' ? '2/5' : '0/5'
    }));
